...
async def read_categories(db: Session = Depends(get_db)
    ...
    return CRUDBase(Transaction).get_all(db)


@router.get("/transactions/page", response_model=Dict)
def get_transactions(
    section: PersonalSectionEnum = Query(
...
def get_transaction(transaction_id: int, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "category": {
            ...
            "name": transaction.category_rel.name,
        } if transaction.category_rel else None,
        "account": {
            "id": transaction.account_rel.id,
            "name": transaction.account_rel.name,
...
def create_transaction(
    ...
    new_transaction = Transaction(
        ...
        credit=True if section in [PersonalSectionEnum.INCOME, PersonalSectionEnum.INVESTMENT] else False, 
        amount=transaction.amount,
        section=section,
    ...
    for idx, transaction_data in enumerate(transactions):
        try:
            ...
            new_transaction = Transaction(
                ...
                section=PersonalSectionEnum(transaction_data['section']),
...
def summary_transactions(
    ...
    income = query.filter(Transaction.section == PersonalSectionEnum.INCOME)\
                 .with_entities(func.coalesce(func.sum(Transaction.amount), 0))\
                 .scalar()
    
    expense = query.filter(Transaction.section == PersonalSectionEnum.EXPENSE)\
                  .with_entities(func.coalesce(func.sum(Transaction.amount), 0))\
                  .scalar()
    
    investment = query.filter(Transaction.section == PersonalSectionEnum.INVESTMENT)\
                     .with_entities(func.coalesce(func.sum(Transaction.amount), 0))\
                     .scalar()
...
def get_spending_by_category(
    ...
    section: PersonalSectionEnum = Query(PersonalSectionEnum.EXPENSE),
    db: Session = Depends(get_db)
):
